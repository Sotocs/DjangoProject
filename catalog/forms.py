...
class ProductForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["description"].widget.attrs.update(
            {"class": "form-control", "rows": 4, "placeholder": "Описание продукта"}
        )
        self.fields["price"].widget.attrs.update(
            {"class": "form-control", "step": "0.01", "placeholder": "Цена"}
        )
    ...
    def clean_name(self):
        ...
        if _contains_forbidden_words(name):
            raise forms.ValidationError(
                "В названии продукта запрещено использовать запрещённые слова."
            )
        return name
    ...
    def clean_description(self):
        ...
        if _contains_forbidden_words(description):
            raise forms.ValidationError(
                "В описании продукта запрещено использовать запрещённые слова."
            )
        return description
